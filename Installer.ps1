# =================================================================
# DWShellNotification - FORCED UI BRIDGE (NO MESSAGEBOX)
# =================================================================
...
# 2. UNLOCK WINDOWS NOTIFICATIONS (Registry Hack)
# This forces Windows to allow "Legacy" bubble notifications
...
# SETUP TRAY ICON (Required for Bubbles to work)
$n = New-Object System.Windows.Forms.NotifyIcon
$n.Icon = [System.Drawing.Icon]::ExtractAssociatedIcon("powershell.exe")
$n.Text = "Shorix Notification Bridge"
$n.Visible = $true
...
while($true) {
    if (Test-Path $trigger) {
        ...
        if ($msg) {
            # Attempt Bubble Notification
            $n.ShowBalloonTip(5000, "Shorix System", $msg.Trim(), "Info")
        }
...
if ($activeUser) {
    ...
    Write-Host "Success. If you still don't see it, check Focus Assist (Do Not Disturb)." -ForegroundColor Green
}
